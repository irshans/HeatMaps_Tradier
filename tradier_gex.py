...
st.markdown("""
    ...
    * { font-family: 'Arial', sans-serif !important; }
    .stButton>button { width: 100%; border-radius: 5px; height: 3em; background-color: #0e1117; color: white; border: 1px solid #30363d; }
...
# -------------------------
def process_institutional_exposure(df, S, s_range):
    ...
    for _, row in df.iterrows():
        ...
        res.append({
            ...
            "gex": gex_val, "vanex": vanex_val, "type": row['option_type'], "oi": oi
        })
...
def render_heatmap(df, ticker, S, mode):
    ...
    for i, strike in enumerate(pivot.index):
        for j, exp in enumerate(pivot.columns):
            ...
            elif abs(val) >= 1e3: label = f"{val/1e3:.0f}K"
            else: label = f"{val:.0f}"
            
            fig.add_annotation(x=exp, y=strike, text=label, showarrow=False, 
                               font=dict(color="white" if val < 0 else "black", size=10))

    fig.update_layout(title=f"{ticker} {mode} Notional Exposure", template="plotly_dark", height=700,
                      xaxis=dict(side='top'), yaxis=dict(title="Strike"))
    return fig
...
def main():
    ...
    max_exp = st.sidebar.slider("Expiries", 1, 10, 5)
    s_range = st.sidebar.slider("Strike Range", 5, 100, 20)
    
    if st.sidebar.button("🔄 Manual Refresh"):
        st.cache_data.clear()
        st.rerun()

    # 10-Minute Auto Refresh Fragment
    @st.fragment(run_every="600s")
    def dashboard():
        now = datetime.now(pytz.timezone('US/Eastern')).strftime("%H:%M:%S")
        st.caption(f"Last Update: {now} EST | Auto-refresh: 10m")
        
        S, raw_df = fetch_data(ticker, max_exp)
        if raw_df is not None:
            df = process_institutional_exposure(raw_df, S, s_range)
            
            c1, c2, c3 = st.columns(3)
            c1.metric("Net GEX", f"${df['gex'].sum()/1e6:.1f}M")
            c2.metric("Net VANEX", f"${df['vanex'].sum()/1e6:.1f}M")
