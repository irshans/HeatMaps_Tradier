...
def render_heatmap(df, ticker, S):
    ...
    for i, strike in enumerate(y_labs):
        for j, exp in enumerate(x_labs):
            val = z_raw[i, j]
            if abs(val) < (abs_limit * 0.1): continue # Only label significant cells
...
def main():
    ...
    if auto_refresh:
        st_autorefresh(interval=60000, key="gex_refresh")
        refresh_col2.write("⏳ Refresh: **60s**")
    refresh_col3.write(f"🕒 Last update: **{datetime.now().strftime('%H:%M:%S')}**")
        ...
        with st.spinner("Processing..."):
            ...
            if S and raw_df is not None:
                ...
                if not df.empty:
                    ...
                    # Charts (Using width="stretch" for 2025 compliance)
